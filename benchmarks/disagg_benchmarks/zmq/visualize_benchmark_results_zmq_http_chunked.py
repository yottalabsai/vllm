...
if __name__ == "__main__":
    ...
    result_df = df.groupby(
        ['name', 'qps'])[keys].agg(lambda x: sum(x) / len(x)).reset_index()
    draw_png(keys, result_df, 'name', 'results/http_zmq_chunk')
    http_zmq_df = result_df[result_df['name'].isin(
        ['disagg_prefill_zmq', 'disagg_prefill_http'])]
    draw_png(keys, http_zmq_df, 'name', 'results/http_zmq')
